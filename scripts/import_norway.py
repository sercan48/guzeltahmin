"""Import Norway Eliteserien matches and run predictions."""
import os
import sys
import requests
import urllib3
...
NOR_RAW_DIR = RAW_DIR / "NORWAY_ELITESERIEN"
NOR_RAW_DIR.mkdir(parents=True, exist_ok=True)

CSV_URL = "http://www.football-data.co.uk/new/NOR.csv"
SCRATCH_CSV = SCRATCH_DIR / "NOR.csv"

def download_norway_csv() -> bool:
    """Download Norway CSV directly or check scratch/NOR.csv mirror."""
    if SCRATCH_CSV.exists() and SCRATCH_CSV.stat().st_size > 20000:
        print(f"[INFO] Found local Norway CSV at {SCRATCH_CSV}. Copying to raw directories...")
        return True

    print(f"Attempting to download Norway CSV from: {CSV_URL}")
    try:
        ...
        if resp.status_code == 200:
            ...
            print("[OK] Norway CSV downloaded successfully and cached in scratch/NOR.csv")
            return True
        else:
...
def main():
    print("=" * 60)
    print("  Norway Eliteserien Data Importer & Predictor")
    print("=" * 60)

    # 1. Download/check CSV
    if not download_norway_csv():
        sys.exit(1)
    ...
    for s in seasons:
        dest_path = NOR_RAW_DIR / f"{s}.csv"
        dest_path.write_bytes(csv_content)
    
    print("[OK] Norway CSV distributed to raw season directories.")

    # 3. Load & Ingest into Database
    db = get_backend()
    db.connect()
    try:
        print("\n[1/2] Loading and parsing Norway Eliteserien 2026 matches...")
        # load_season will filter only 2026 matches because of our custom logic
        df = load_season("2526", "NORWAY_ELITESERIEN")
        
        if df is None or len(df) == 0:
            print("[WARN] No Norway Eliteserien matches found for 2026 season.")
            return

        print(f"Loaded {len(df)} matches for Norway Eliteserien 2026 season.")
        
        # Ingest into database
        print("[2/2] Ingesting matches to database...")
        inserted = ingest_matches_to_db(df, db)
        print(f"[OK] Ingested/updated {inserted} Norway Eliteserien matches in database.")
